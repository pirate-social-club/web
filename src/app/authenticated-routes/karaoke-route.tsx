...
function KaraokeAuthRequiredMessage({ postId }: { postId: string }) {
  ...
  return (
    <div className="flex h-dvh min-h-screen w-full items-center justify-center bg-background px-6 text-foreground">
      <div className="flex max-w-sm flex-col items-center gap-4 text-center">
        <Type as="h1" variant="h3">
          Sign in to use karaoke
        </Type>
        <Type as="p" className="text-muted-foreground" variant="body">
          Karaoke requires a Pirate account.
        </Type>
        {configured && connect ? (
...
export function KaraokeRoutePage({ postId }: { postId: string }) {
  ...
  if (!session?.accessToken || state.phase === "auth_required") {
    return <KaraokeAuthRequiredMessage postId={postId} />;
  }
