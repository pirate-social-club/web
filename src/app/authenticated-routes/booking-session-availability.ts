...
// Party-triggered settlement is attendance-derived. The API accepts both actions only after the
// scheduled session ends, so neither control may imply that leaving the call moves money early.
 ...
 * This schedules a one-shot timer to slot_end, re-rendering exactly when it elapses, and tears the
 * timer down on unmount / booking change.
 */
