...
function FileComposerFlow(props: {
  ...
  return (
    <ComposerFrame narrow>
      <PostComposer
        ...
        mode="file"
        availableTabs={["file"]}
        titleValue={props.title ?? "Research export"}
        textBodyValue={props.textBody ?? "A deterministic downloadable file."}
        composerStep={step()}
...
export const StoryRetry: Story = {
  name: "Retrying — registration (StoryRetry)",
  render: () => (
    <FileComposerFlow
      initialStep="publish"
      initialStatus="Retrying publication through the compatibility StoryRetry path"
      title="Compatibility retry registration"
      textBody="A downloadable file retry surfaced through the legacy StoryRetry catalog name."
    />
  ),
};
