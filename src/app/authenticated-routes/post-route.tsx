...
export function PostPage({
  ...
  const { connectedWallets } = usePiratePrivyWallets({
    enabled: Boolean(
      session?.accessToken
        && (activeLiveRoomId || activeAssetPostType === "file" || activeAssetPostType === "deck"),
    ),
  });
  ...
  const commerceEnabled = Boolean(
    session?.user?.id
      && community?.id
      && (
        ((post?.post.post_type === "song" || post?.post.post_type === "video" || post?.post.post_type === "file" || post?.post.post_type === "deck") && post.post.asset)
          || activeLiveRoomId
      ),
  );
  const {
  ...
  const songOptions = (post.post.post_type === "song" || post.post.post_type === "video") && community && threadAssetId
    ...
    : undefined;
  const genericAssetOptions = (post.post.post_type === "file" || post.post.post_type === "deck") && community && threadAssetId
    ? {
      genericAsset: {
        accessState: threadPurchase ? "available" as const : threadListing ? "purchase_required" as const : "unknown" as const,
        hasEntitlement: Boolean(threadPurchase),
        ...
        priceLabel: threadListing ? `$${(threadListing.price_cents / 100).toFixed(2)}` : undefined,
      },
    }
    : undefined;
