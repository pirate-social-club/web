...
function StoryRegistrationIcon(props: { state: StoryRegistrationStatus["state"] }) {
  return (
    <Switch fallback={<IconWarningCircle aria-hidden="true" class="mt-0.5 size-4 shrink-0" />}>
      <Match when={props.state === "registered"}>
        <IconCheckCircle aria-hidden="true" class="mt-0.5 size-4 shrink-0" />
      </Match>
      <Match when={props.state === "pending"}>
        <IconClock aria-hidden="true" class="mt-0.5 size-4 shrink-0" />
      </Match>
    </Switch>
  );
}
