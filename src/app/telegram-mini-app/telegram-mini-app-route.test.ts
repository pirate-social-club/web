...
describe("telegramVerifyTerminalMessage", () => {
  ...
  test("uses short requirement copy for nationality gate failures", () => {
    const eligibility = {
      failure_reason: "nationality_mismatch",
      gate_evaluation: null,
      membership_gate_summaries: [{ gate_type: "nationality", required_value: "US" }],
      status: "gate_failed",
    } as Parameters<typeof telegramVerifyTerminalMessage>[0];

    expect(telegramVerifyTerminalMessage(eligibility, "en")).toBe("Required: United States nationality");
  });

  test("uses short requirement copy for minimum age gate failures", () => {
    const eligibility = {
      failure_reason: "minimum_age_mismatch",
      gate_evaluation: null,
      membership_gate_summaries: [{ gate_type: "minimum_age", required_minimum_age: 21 }],
      status: "gate_failed",
    } as Parameters<typeof telegramVerifyTerminalMessage>[0];

    expect(telegramVerifyTerminalMessage(eligibility, "en")).toBe("Required: 21+ ID check");
  });
});
...
describe("telegram verification view model", () => {
  ...
  test("treats status checks as busy", () => {
  ...
  test("uses structured done results instead of message matching", () => {
    expect(resolveTelegramVerifyViewModel({
