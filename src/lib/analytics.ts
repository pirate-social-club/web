...
export function trackAnalyticsEvent(input: TrackAnalyticsEventInput): void {
  ...
  void fetch(resolveApiUrl("/analytics/events", window.location.hostname), {
    body: JSON.stringify({
      ...
      community: input.communityId ?? undefined,
      post: input.postId ?? undefined,
      comment: input.commentId ?? undefined,
      listing: input.listingId ?? undefined,
      quote: input.quoteId ?? undefined,
      purchase_id: input.purchaseId ?? undefined,
      id: input.verificationSessionId ?? undefined,
