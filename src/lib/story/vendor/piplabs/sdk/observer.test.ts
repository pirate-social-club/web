...
function createObserverWithRegisteredLogs(params: {
  latestBlock: bigint;
    ...
    getLogs: async ({
      ...
      if (
        ...
        return [
          makeRegisteredLog({
            validatorAddr: params.validatorAddr,
            enclaveReport: params.enclaveReport,
...
describe("Observer registered-event lookback", () => {
  test("finds validator attestations more than 50k blocks behind the tip", async () => {
    const ranges: Array<{ fromBlock: bigint; toBlock: bigint }> = [];
    const validatorAddr = "0x1234567890abcdef1234567890abcdef12345678" as Hex;
    const enclaveReport = "0xa1b2c3d4" as Hex;
    const enclaveCommKey = "0x01020304" as Hex;
    const latestBlock = 17_070_000n;
    const registrationBlock = 16_954_084n;
    const observer = createObserverWithRegisteredLogs({
      latestBlock,
      registrationBlock,
      validatorAddr,
      enclaveReport,
  ...
  test("falls back to a genesis scan when the default lookback finds no attestations", async () => {
    const ranges: Array<{ fromBlock: bigint; toBlock: bigint }> = [];
    const validatorAddr = "0xabcdefabcdefabcdefabcdefabcdefabcdefabcd" as Hex;
    const enclaveReport = "0x0a0b0c0d" as Hex;
    const enclaveCommKey = "0x05060708" as Hex;
