...
function walletRewardsSummary(input: {
  ...
  if (input.rewardsCashoutPending) {
    return {
      ...
      assetLabel,
      pending: true,
      supportingLabel: "Preparing your claim.",
    };
  }
  ...
  if (input.rewards.cashout.eligible) {
    return {
      actionLabel: `Claim ${formatRewardBalanceCents(input.rewards.balance_cents)}`,
      amountLabel,
      assetLabel,
      onAction: input.onMoveToWallet,
      supportingLabel: "To your Pirate wallet.",
    };
  }
...
export function CurrentUserWalletPage() {
  ...
  const {
    checkPendingVerification: checkRewardsZkPassport,
    clearPendingVerification: clearRewardsZkPassport,
    startVerification: startRewardsZkPassportVerification,
    verificationError: rewardsZkPassportError,
    verificationHref: rewardsZkPassportHref,
    verificationLoading: rewardsZkPassportLoading,
  ...
  React.useEffect(() => {
    const message = selfError || veryRewardsError || rewardsZkPassportError;
    if (!message) return;
  ...
  const openRewardsCashout = React.useCallback(() => {
    void handleRewardsCashout(rewardsSummary);
  }, [handleRewardsCashout, rewardsSummary]);
  ...
  React.useEffect(() => {
    ...
    if (!cashoutId || rewardsCashoutState === "confirmed" || rewardsCashoutState === "failed") return;
    let cancelled = false;
    let timeout: number | undefined;
    ...
    <StandardRoutePage size="rail">
      ...
      {rewardsEnabled ? (
        <CashoutSheet
          ...
          basescanUrl={baseTxUrl(rewardsCashoutTxHash, rewardsCashoutState)}
      ...
      <ZkPassportVerificationModal
        ...
        onOpenChange={(open) => {
          setRewardsZkPassportOpen(open);
          if (!open) clearRewardsZkPassport();
        }}
        open={rewardsZkPassportOpen}
