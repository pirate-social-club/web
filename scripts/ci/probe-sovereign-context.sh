...
probe_apex_headers_file="$(mktemp)"
namespace_file="$(mktemp)"
trap 'rm -f "$html_file" "$app_html_file" "$canonical_html_file" "$canonical_threads_html_file" "$inventory_apex_html_file" "$inventory_app_html_file" "$inventory_apex_headers_file" "$probe_apex_headers_file" "$namespace_file"' EXIT
...
apex_redirect="$(request_redirect_status "$HNS_PROBE_ROOT" "/" "$probe_apex_headers_file")"
