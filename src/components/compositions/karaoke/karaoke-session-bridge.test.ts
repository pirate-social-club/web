...
describe("createKaraokeSessionClient", () => {
  ...
  test("reconnect reuses the idempotency key and keeps the same attempt after a socket drop", async () => {
    const h = harness({ override: { reconnectDelayMs: 1 } });
    await h.handle.start();
    h.sockets[0]!.open();
    await tick();
    expect(h.handle.getPhase()).toBe("live");

    h.sockets[0]!.close(1006, "network drop");
    await h.timers.flushNext();
  ...
  test("identity drift on reconnect is terminal", async () => {
    const h = harness({
      override: { reconnectDelayMs: 1 },
      responses: (call) => apiResponse({
        attempt: call === 1 ? "attempt-1" : "attempt-2",
        session_expires_at: Math.floor(BASE_NOW / 1000) + 3600,
        token_expires_at: Math.floor(BASE_NOW / 1000) + 60,
      }),
    });
