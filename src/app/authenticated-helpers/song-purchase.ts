...
type PurchaseAssetLabel = "song" | "video" | "file" | "deck" | "ticket" | "replay" | "asset";
...
export function useSongPurchaseFlow({
  ...
  const buySong = React.useCallback(async (params: PendingSongPurchase) => {
    ...
    if (!fundingWallet) {
      toast.error(`Connect your primary wallet before buying this ${assetLabel}.`);
      return;
    }
  ...
  const confirmPurchase = React.useCallback(async () => {
    ...
    if (!fundingWallet) {
      setPurchaseError(`Connect your primary wallet before buying this ${assetLabel}.`);
      return;
    }
