...
import { mergeSeededVideoItems } from "@/app/video-experience/merge-seeded-video-items";

export interface VideoExperienceOpenRequest {
  id: number;
  seed: VideoExperienceSeed;
}

export function VideoExperienceOverlay({ request }: { request: VideoExperienceOpenRequest | null }) {
  const api = useApi();
  ...
  React.useEffect(() => {
    ...
    latestSeedRef.current = request.seed;
    seedRequestGenerationRef.current += 1;
    setLoadingSeed(false);
    installSeed(request.seed);
  }, [installSeed, request]);

  React.useEffect(() => {
    if (!request) syncFromLocation();
    window.addEventListener("popstate", syncFromLocation);
    return () => window.removeEventListener("popstate", syncFromLocation);
  }, [request, syncFromLocation]);
