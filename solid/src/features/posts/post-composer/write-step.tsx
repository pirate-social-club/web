...
export function PostComposerWriteStep(props: { controller: PostComposerController }) {
  ...
  const body = (mobile: boolean) => (
    <>
      <Show when={!mobile}>
        <Input maxlength={300} onInput={(event) => controller.fields.onTitleValueChange?.(event.currentTarget.value)} placeholder="Title*" size="title" value={controller.fields.titleValue} />
      </Show>
      <Show when={mobile}>
        <Textarea class="min-h-18 resize-none rounded-none border-0 bg-transparent p-0 text-3xl font-semibold leading-tight shadow-none focus-visible:ring-0" maxlength={300} onInput={(event) => controller.fields.onTitleValueChange?.(event.currentTarget.value)} placeholder={controller.copy.placeholders.title} value={controller.fields.titleValue} />
      </Show>
      ...
      <Textarea class={cn("resize-none text-xl leading-relaxed", mobile ? "min-h-[38dvh] rounded-none border-0 bg-transparent p-0 shadow-none focus-visible:ring-0" : "min-h-36")} onInput={(event) => updateBody(controller, event.currentTarget.value)} placeholder={attachment() ? controller.copy.placeholders.optional : controller.copy.placeholders.body} value={bodyValue(controller)} />
