...
function usage(): string {
  return [
    ...
    "  PIRATE_CHECKOUT_OPERATOR_PRIVATE_KEY or PIRATE_STORY_E2E_BUYER_PRIVATE_KEY",
...
async function decryptCdrAsset(input: {
  ...
  const partials = await cdrClient.consumer.collectPartials({
    ...
    fromBlock,
    requesterPubKey,
    timeoutMs: Number(optionalEnv("PIRATE_STORY_E2E_CDR_TIMEOUT_MS") ?? "90000"),
  });
  const partialTxHashes = await collectCdrPartialTxHashes({
