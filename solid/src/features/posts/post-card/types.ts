// Domain-aligned types ported from the React post-card (post-card.types.ts).
// Differences from the React source:
// - `viewerAttachResponse`/`onViewerRenew` are omitted: the inline live-room
//   viewer surface (LiveRoomViewerSurface) belongs to the live-room-viewer
//   lane, not B7a. The post-page surface renders the cover instead.
// - `streakSummary` is omitted: the streak preview (SongStreakPreview) belongs
//   to song-study. `streaksHref`/`onStreaks` are kept so callers can wire the
//   entry points; without a summary nothing renders, matching React.
// - `PostCardMenuItem.icon` is a plain marker (`PostCardMenuIcon`) instead of
//   JSX so menu derivation stays a pure, renderer-agnostic module.
...
export type PostCardShareAction = PostCardMenuItem & {
...
export interface SongContentSpec {
  ...
  // Streak leaderboard entry points. The inline streak section
  // (SongStreakPreview) is a song-study surface and is not part of B7a.
  streaksHref?: string;
  onStreaks?: () => void;
...
export interface PostCardProps {
  ...
  /**
   * Called with the post href when the card body is activated (click or
   * Enter). Replaces the React version's direct `@/app/router` import so
   * stories and tests stay router-free.
   */
