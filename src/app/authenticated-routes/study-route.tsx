...
import { SongStudySurface } from "@/components/compositions/song-study/song-study-surface";
...
import {
  ...
  caughtUpMessage,
  exerciseSurface,
  type FillBlankSurfaceState,
  LEGACY_SAY_IT_BACK_ATTEMPTS_PER_APPEARANCE,
  lockedSurface,
  type MultipleChoiceSurfaceState,
  pageArtwork,
  pageTitle,
  type ReadyStudyRouteState,
...
function SongStudyRoutePage({
  ...
  const pendingFillBlankAttemptRef = React.useRef<string | null>(null);
  ...
  React.useEffect(() => {
    ...
    async function loadStudy() {
      ...
      try {
        ...
        const authoritativeNext = usesFillBlankOrchestration(study)
          ? study.lesson?.next?.prompt
          : undefined;
        const firstIndex = authoritativeNext
          ? study.exercises.findIndex((exercise) => exercise.id === authoritativeNext.id)
          : exerciseQueue[0];
        ...
        setState({
          ...
          rewardOffer,
          study,
          surface: authoritativeNext
            ? exerciseSurface(authoritativeNext, study.lesson!.next!.presentation_number)
            : exerciseSurface(study.exercises[firstIndex]!),
        });
        preloadStudyFeedbackSounds();
      } catch (error) {
        if (canceled) return;
  ...
  const submitMultipleChoiceAttempt = React.useCallback((
    ...
    void api.communities.submitPostStudyAttempt(readyState.post.post.community, readyState.post.post.id, {
      ...
      ...(usesFillBlankOrchestration(readyState.study) && readyState.study.lesson
        ? { session_revision: readyState.study.lesson.session_revision }
        : {}),
  ...
  const submitFillBlankAttempt = React.useCallback((
    readyState: ReadyStudyRouteState,
    surface: FillBlankSurfaceState,
  ) => {
    if (surface.result || surface.submitting) return;
    const blankIds = surface.exercise.segments.flatMap((segment) => segment.kind === "blank" ? [segment.id] : []);
    if (blankIds.length === 0 || surface.selectedTokenIds.length !== blankIds.length) return;
    const studySessionId = readyState.study.session?.id;
    if (!studySessionId) return;
    const pendingKey = `${surface.exercise.id}:${surface.attemptNumber}`;
    if (pendingFillBlankAttemptRef.current === pendingKey) return;
    pendingFillBlankAttemptRef.current = pendingKey;
    setState((current) => current.phase === "ready"
      && current.surface.kind === "fill_blank"
      && current.surface.exercise.id === surface.exercise.id
      ? { ...current, surface: { ...current.surface, submitError: undefined, submitting: true } }
      : current);
    void api.communities.submitPostStudyAttempt(readyState.post.post.community, readyState.post.post.id, {
      attempt_number: surface.attemptNumber,
      exercise_id: surface.exercise.id,
      idempotency_key: attemptIdempotencyKey(studySessionId, surface.exercise.id, surface.attemptNumber),
      placements: blankIds.map((blankId, index) => ({ blank_id: blankId, token_id: surface.selectedTokenIds[index]! })),
      session_id: studySessionId,
      ...(readyState.study.lesson ? { session_revision: readyState.study.lesson.session_revision } : {}),
      timezone: deviceTimezone(),
      type: "fill_blank",
    }).then((result) => {
      pendingFillBlankAttemptRef.current = null;
      divergenceRecoveryCountRef.current = 0;
      playStudyFeedbackSound(result.outcome === "correct" ? "correct" : "incorrect");
      setState((current) => current.phase === "ready"
        && current.surface.kind === "fill_blank"
        ...
        ? {
            ...
            surface: {
              ...current.surface,
              correctPlacements: result.correct_placements,
              result: result.outcome === "correct" ? "correct" : "wrong",
              submitting: false,
            },
          }
      ...
      pendingFillBlankAttemptRef.current = null;
      if (isStudyAttemptDivergence(error) && recoverFromDivergedAttempt()) return;
      setState((current) => current.phase === "ready"
        && current.surface.kind === "fill_blank"
        && current.surface.exercise.id === surface.exercise.id
        ? { ...current, surface: { ...current.surface, submitError: getErrorMessage(error, "Could not record this answer. Try again."), submitting: false } }
        : current);
    });
  ...
  const handlePrimaryAction = React.useCallback(() => {
    ...
    if (state.surface.kind === "fill_blank") {
      if (state.surface.result) {
        setState(advanceLesson(state, state.surface.result));
      } else {
        submitFillBlankAttempt(state, state.surface);
      }
      return;
    }
          ...
          recorder.onstop = () => {
            ...
            void api.communities.transcribePostStudyAudio(state.post.post.community, state.post.post.id, {
              ...
              throw new StudyTranscriptionFailure(error);
                ...
                session_id: studySessionId,
                ...(usesFillBlankOrchestration(state.study) && state.study.lesson
                  ? { session_revision: state.study.lesson.session_revision }
                  : {}),
                timezone: deviceTimezone(),
              ...
              .then(({ result, transcript }) => {
                ...
                setState((current) => {
                  ...
                  const authoritative = usesFillBlankOrchestration(current.study);
                  const retryInPlace = authoritative && result.lesson
                    ? result.lesson.next?.retry_in_place === true && result.lesson.next.exercise_id === exercise.id
                    : (result.attempts_remaining ?? 0) > 0
                      && attemptsUsed < LEGACY_SAY_IT_BACK_ATTEMPTS_PER_APPEARANCE;
                  const spent = !retryInPlace;
                  return {
                    ...current,
                    lastAttemptResult: result,
                    study: {
                      ...current.study,
                      ...(authoritative && result.lesson ? { lesson: result.lesson } : {}),
                      ...(authoritative && result.session ? { session: result.session } : {}),
                    },
                    surface: {
                      ...current.surface,
                      attemptNumber: retryInPlace
                        ? authoritative
                          ? result.lesson?.next?.presentation_number ?? current.surface.attemptNumber + 1
                      ...
                      submitError: undefined,
                      willReturn: authoritative && result.lesson
                        ? Boolean(result.lesson.next)
                        : (result.attempts_remaining ?? 0) > 0
  ...
  }, [api, attemptIdempotencyKey, postId, recoverFromDivergedAttempt, state, stopRecordingStream, submitFillBlankAttempt, submitMultipleChoiceAttempt, telegramMiniApp]);
  ...
  const handleFillBlankTokenSelect = React.useCallback((tokenId: string) => {
    setState((current) => {
      if (current.phase !== "ready" || current.surface.kind !== "fill_blank" || current.surface.result || current.surface.submitting) return current;
      const blankCount = current.surface.exercise.segments.filter((segment) => segment.kind === "blank").length;
      if (current.surface.selectedTokenIds.includes(tokenId) || current.surface.selectedTokenIds.length >= blankCount) return current;
      return { ...current, surface: { ...current.surface, selectedTokenIds: [...current.surface.selectedTokenIds, tokenId] } };
    });
  }, []);

  const handleFillBlankUndo = React.useCallback(() => {
    setState((current) => current.phase === "ready" && current.surface.kind === "fill_blank" && !current.surface.result
      ? { ...current, surface: { ...current.surface, selectedTokenIds: current.surface.selectedTokenIds.slice(0, -1) } }
      : current);
  }, []);

  const handleFillBlankClear = React.useCallback(() => {
    setState((current) => current.phase === "ready" && current.surface.kind === "fill_blank" && !current.surface.result
      ? { ...current, surface: { ...current.surface, selectedTokenIds: [] } }
      : current);
  }, []);
  ...
  return (
    <SongStudySurface
      ...
      lessonProgress={state.phase === "ready"
        ? usesFillBlankOrchestration(state.study) && state.study.lesson
          ? { resolvedCount: state.study.lesson.resolved_count, totalCount: state.study.lesson.total_count }
          : studyLessonProgress({
      ...
      onFillBlankClear={handleFillBlankClear}
      onFillBlankTokenSelect={handleFillBlankTokenSelect}
      onFillBlankUndo={handleFillBlankUndo}
